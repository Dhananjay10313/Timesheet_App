...
def get_timsheet_data_by_user(current_user: userBase, db: Session = Depends(get_db)):
    ...
    timsheet_data_user = db.query(Timesheet).filter(Timesheet.employee_id == emp_id)
...
def get_leave_data_by_user(req:RequestBody, db: Session = Depends(get_db)):
    try:
        ...
        leave_data_Creatoruser = db.query(Leaves).filter(Leaves.employee_id == emp_id).all()
        # ticket_data_Refuser = db.query(Ticket).filter(Ticket.ref_employee_id == emp_id)
...
def get_leave_remaining(req:RequestBody, db: Session = Depends(get_db)):
    emp_id=req.id
    ...
    for ticket in leave_remaining:
        result_list.append({
            "employee_id": typeOfLeave.employee_id,
            "t_id":typeOfLeave.t_id,
            "t_days":typeOfLeave.t_days,
            "balance":typeOfLeave.balance
        })
